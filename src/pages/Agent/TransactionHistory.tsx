...
export default function TransactionHistory() {
    ...
    const { data: transInfo } = useGetMyTransactionQuery(undefined);
                    ...
                    <tbody>
                        {transactions.length > 0 ? (
                            transactions.map((tx: any, index: number) => (
                                <tr
                                    key={tx._id}
                                    className={`border-t hover:bg-primary/10 ${index % 2 === 0 ? "" : ""
                                        }`}
                                >
                                    ...
                                    <td className="px-4 py-2">
                                        <span
                                            className={`px-2 py-1 rounded-full text-xs font-semibold ${tx.status === "SUCCESS"
